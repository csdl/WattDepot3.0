...
public class WattDepotImpl extends WattDepot {
  ...
  public WattDepotImpl() {
    ...
    UserInfo a = (UserInfo) session.get(UserInfo.class, UserInfo.ADMIN.getId());
    if (a == null) {
    ...
    session.beginTransaction();
    UserGroup ua = (UserGroup) session.get(UserGroup.class, UserGroup.ADMIN_GROUP.getId());
    if (ua == null) {
  ...
  /* (non-Javadoc)
   * @see org.wattdepot3.server.WattDepot#updateUserGroup(org.wattdepot3.datamodel.UserGroup)
   */
  @Override
